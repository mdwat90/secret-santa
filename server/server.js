    ...
    function (err, res) {
      if (err) {
        ...
        const msg = {
          ...
          from: 'secret-santa-app@secret-santa-19.herokuapp.com',
          subject: `Password Reset`,
          text: `${upperCaseName}, your password for the Secret Santa app has been reset.`,
          html: `<p>${upperCaseName}, your password for the Secret Santa app has been reset successfully.</p>
            <br>
            <strong>If you did not authorize this request, you can reset your password <a href="https://secret-santa-19.herokuapp.com/reset-password">here</a>.</strong>
            `,
        };
        sgMail.send(msg);
      }
            ...
            resp.map((group, index) => {
              group.members.map((member, index) => {
                if (member.uid === item.user_id) {
                  ...
                  upperCaseName = userName.replace(/^\w/, (c) =>
                    c.toUpperCase()
                  );
                }
              });
        ...
        GroupSchema.findOneAndUpdate(
          ...
          function (errs, resp) {
            ...
            if (resp === null) {
              ...
              let userName;
              let upperCaseGroup = resp.name.replace(/^\w/, (c) =>
                c.toUpperCase()
              );
              let upperCaseName = request.name.replace(/^\w/, (c) =>
                c.toUpperCase()
              );
...
app.get('/api/getUserGroups', function (req, response) {
  ...
  GroupSchema.find(
    { $or: [{ admin: user_id }, { 'members.uid': { $eq: user_id } }] },
    function (err, res) {
      if (res === null) {
