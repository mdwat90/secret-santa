      ...
      <div>
        <Card className={classes.card} style={{ minHeight: '30vh' }} key={idx}>
          <Grid container>
            <Grid item style={{ flex: 1 }}>
              <Typography variant="h4" className={classes.title} key={idx}>
                {upperCaseGroup}
              </Typography>
            </Grid>
            {group.admin === user_info._id ? (
              <Grid
                item
                style={{ position: 'relative', right: 25, bottom: 10 }}
              >
                <SettingsIcon
                  ...
                  aria-haspopup="true"
                  onClick={(event) => this.openOptionsMenu(event)}
                />
                <Menu
                  id="simple-menu"
                ...
                >
                  <MenuItem
                    onClick={() =>
                      this.openResetDrawingModal(this.state.groupId)
                    }
                  >
                    Reset Drawing
                  </MenuItem>
                  <MenuItem
                    onClick={() =>
                      this.openDeleteGroupModal(this.state.groupId)
                    }
          ...
          {group.memberCount !== 0 ? (
            group.memberCount === 1 ? (
              <Typography variant="body1" key={idx}>
                {group.memberCount} spot left
              </Typography>
            ) : (
              <Typography variant="body1" key={idx}>
                {group.memberCount} spots left
              </Typography>
            )
          ) : null}
          ...
          <Accordion className={classes.panel}>
            ...
            <AccordionDetails>
              <Grid container wrap="wrap" direction="row" justify="flex-start">
                {group.members.map((member, index) => {
                  //member is NOT current user and current user is the group admin
                  if (
                    member.uid !== user_info._id &&
                    group.admin === user_info._id
                  ) {
                    return (
                      <div key={index}>
                        {member.selectedBy === user_info._id ? (
                          <Grid item style={{ margin: '0.3em' }}>
                            <Chip
                              ...
                              onClick={() =>
                                this.props.routeChange(member.uid, member.name)
                              }
                              style={{
          ...
          {group.memberCount !== 0 ? (
            <div>
              <Typography variant="body1" className={classes.subText}>
                Once everyone joins the group, you can draw a name here.
              </Typography>
            </div>
          ) : null}
              ...
              return (
                <Container key={index}>
                  ...
                  <Typography className={classes.subText}>
                    You can click their name to see their wishlist.
                  </Typography>
                </Container>
              );
            }
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              ...
              <Typography style={{ textAlign: 'center', fontWeight: 'bold' }}>
                This will reset the drawing for the entire group.
              </Typography>
            </DialogContentText>
          </DialogContent>
          <DialogActions>
            ...
            <Button
              onClick={() =>
                this.removeMember(this.state.groupId, this.state.deleteUid)
              }
              color="secondary"
            >
              Confirm
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              <Typography style={{ textAlign: 'center' }}>
                {' '}
                Are you sure you want to reset the drawing for this group?{' '}
              </Typography>
            </DialogContentText>
          </DialogContent>
          <DialogActions>
            <Button
              onClick={() => this.closeResetDrawingModal()}
              style={{ color: '#6b6b6b' }}
              autoFocus
            >
              Close
            ...
            <Button
              onClick={() => this.clearSelections(this.state.groupId, false)}
              color="secondary"
            >
              Reset
            </Button>
        ...
        >
          ...
          <DialogActions>
            ...
            <Button
              onClick={() => this.deleteGroup(this.state.groupId)}
              color="secondary"
            >
              Delete
            </Button>
